...
export class ButtonComponent {
  ...
  getButtonClasses(): string {
    ...
    const loadingClasses = this.isLoading ? 'opacity-75 pointer-events-none cursor-default' : '';

    return `${baseClasses} ${sizeClasses} ${styleClasses} ${loadingClasses} ${this.customClass}`.trim();
  }
}
